...
import { AccountNotFoundException } from "@tsalliance/rest";
import { SSOService } from "@tsalliance/sso-nest";
import { Socket } from "socket.io"
import { UploadedAudioFile } from "../entities/uploaded-file.entity";

@WebSocketGateway({ 
    namespace: "/upload-status/",
    cors: {
        origin: "*",
        credentials: true
    }
})
export class UploadStatusGateway implements OnGatewayConnection, OnGatewayDisconnect {
     ...
     * @param uploadedFile 
     */
    public sendStatusToUploader(uploadedFile: UploadedAudioFile) {
        try {
            this.findSocketsByUserId(uploadedFile.uploader.id).then((sockets) => {
                for(const socket of sockets) {
                    socket.emit("onStatusChanged", uploadedFile)
                }
            });
        } catch (error) {
            console.error(error)
    ...
    public async handleConnection(client: Socket): Promise<void> {
        ...
        try {
            ...
            // Fetch user data by provided auth header
            if(!user) {
                this.sendError(client, new AccountNotFoundException(), true)
                return;
            }
